...
from aiida_vibroscopy.calculations.spectra_utils import elaborate_tensors
from aiida_vibroscopy.utils.validation import validate_matrix, validate_nac
        ...
        if 'preprocess_data' in self.inputs:
            ...
            if 'displacement_generator' in self.inputs.phonon_workchain:
                preprocess = preprocess.calcfunctions.get_preprocess_with_new_displacements(
                    self.inputs.phonon_workchain.displacement_generator
                )
        else:
            preprocess_inputs = {'structure': self.inputs.structure}
            for input_ in [
                'supercell_matrix', 'primitive_matrix', 'symprec', 'is_symmetry', 'displacement_generator',
                'distinguish_kinds'
            ]:
                if input_ in self.inputs.phonon_workchain:
                    preprocess_inputs.update({input_: self.inputs.phonon_workchain[input_]})
            ...
            if nspin != 1:
                if len(preprocess.get_unitcell().sites) != len(preprocess.get_primitive_cell().sites):
                    raise NotImplementedError(
                        'a primitive cell smaller than the unitcell with spin polarized is not supported.'
                    )
        else:
