...
def voigt_profile(x_range: np.ndarray, peak: float, intensity: float, gamma_lorentz: float, sigma_gaussian: float):
    ...
    for index, i in enumerate(list_a):
        ...
        w_L = w_L - (1.0 - rho) * list_b[index] * (rho**i)
        w_I = w_I + list_c[index] * (rho**i)
        w_P = w_P + list_d[index] * (rho**i)
        eta_L = eta_L + rho * (1.0 - rho) * list_f[index] * (rho**i)
        eta_I = eta_I + rho * (1.0 - rho) * list_g[index] * (rho**i)
        eta_P = eta_P + rho * (1.0 - rho) * list_h[index] * (rho**i)
    #
    #
    big_W_G = w_G * (f_G + f_L)
    big_W_L = w_L * (f_G + f_L)
    ...
    gamma_L = big_W_L / 2.0
    gamma_I = big_W_I / (2.0 * (2 ** (2.0 / 3.0) - 1.0) ** 0.5)
    # this parameter has been checked numerically, the original paper has a typo
    # in the formula for gamma_P
    gamma_P = big_W_P / (2.0 * (np.log(np.sqrt(2.0) + 1.0)))
    #
    # this because the paper uses a rescaled sigma.
    res_Gauss = np.sqrt(1.0 / np.pi) * (1.0 / gamma_G) * np.exp(-((x / gamma_G) ** 2))
    #
    res_Lorentzian = (1.0 / np.pi) * gamma_L / (x**2 + gamma_L**2)
    #
    res_f_I = 1.0 / (2.0 * gamma_I) * ((1.0 + (x / gamma_I) ** 2) ** (-3.0 / 2.0))
    #
    res_f_P = 1.0 / (2.0 * gamma_P) * (2.0 / (np.exp(x / gamma_P) + np.exp(-x / gamma_P))) ** 2
    #
    res = (1.0 - eta_L - eta_I - eta_P) * res_Gauss + eta_L * res_Lorentzian + eta_I * res_f_I + eta_P * res_f_P
