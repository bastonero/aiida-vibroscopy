...
```console
$ pytest tests/calculations/test_spectra.py
```
...
```console
$ uv run pytest tests/calculations/test_spectra.py
```
...
```console
$ hatch test tests/calculations/test_spectra.py
```
...
In the following, we assume that the latest release was `v1.6.1`.
Furthermore, we assume the following naming conventions for remotes:

* `origin`: remote pointing to `bastonero/aiida-vibroscopy`
* `fork`: remote pointing to personal fork
...
* Patch release: `v1.6.1` to `v1.6.2`, only bug fixes
* Minor release: `v1.6.1` to `v1.7.0`, bug fixes and new features that **maintain** backwards compatibility
* Major release: `v1.6.1` to `v2.0.0`, bug fixes and new features that **break** backwards compatibility
...
For most releases, we assume that all the changes in the current `main` branch have to be included in the release. As such, branch off the new release branch directly from `main`:
    ...
    git fetch --all --prune
    git checkout origin/main -b release/1.7.0

#### Updating the `CHANGELOG.md` and version
...
One final check you should perform:
...
Once you've prepared the release branch locally, commit with the message 'Release `v1.7.0`' and push it to Github. For our major/minor release example:
...
Make sure the remote `release/1.7.0` branch is up to date by pushing the local changes, then go to Github and create a pull request to the `main` branch of the official repository.

After the pull request has been approved, merge the PR using the "Squash and Merge", and make sure the commit is simply named e.g. 'Release `v1.7.0`'.
...
:::{important}
In case you did push with the wrong tag, the CD should fail since we [validate the tag](https://github.com/bastonero/aiida-vibroscopy/blob/main/.github/workflows/validate_release_tag.py) as part of the workflow.
