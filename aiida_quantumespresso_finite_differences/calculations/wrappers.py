...
def wrap_tensors(epsilon, born_charges):
    ...
    results.array.set_array('epsilon', epsilon.get_array('epsilon'))
    results.array.set_array('born_charges', born_charges.get_array('born_charges'))
    
    return results
    
@workfunction
def run_tensors(outputs_null, outputs_finites, E):
    """Work function to compute the output_tensors."""
    epsilon = compute_high_frequency_dielectric_tensor(outputs_null, outputs_finites, E)
...
@workfunction
def run_arrays(outputs_null, outputs_finites, E, direction):
    """Work function to compute the output_arrays."""
    epsilon = compute_high_frequency_dielectric_array(outputs_null, outputs_finites, E, direction)
